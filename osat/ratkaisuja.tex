...
esitysmalliin liittyvät rajoitukset. Käytännössä tähän ei päästä, koska
rajanpinta tiedon käsittelyn logiikan ja sovelluslogiikan välillä on
toteutettu SQL"-kielellä. Monien tietokantojen tuki SQL"-kielelle on
varsin kehittymätön erityisesti tietorakenteen rajoitusten suhteen.
Tämän seurauksena merkittävä osa tiedon esitysmallin rajoituksista
joudutaan sijoittamaan sovelluslogiikkaan. Muuten sovellusta ei voida
...
otsake sisälti tunnisteen \code{no-cache}. Tämä tunniste kieltää
...
esimerkiksi normaalille selaimelle, dataprojektorille, ääniselaimille
ja kämmentietokoneille; käyttöliittymän esitystä on siis mahdollista
hioa tarvittaessa laitekohtaisesti.
...
\code{Document}"-luokasta, joka auttoi sessiokohtaisen tiedon
...
tulevaisuudesssa useita dokumenttityyppejä voidaan tarvittaessa ajaa
...
XML"-muodossa. Sovellus generoi siis XHTML"-kielisen esityksen ja
käyttöliittymäajurin rajapinta generoikin suoraan XML"-kielisiä palasia
...
ajava edustakone ei tallenna mitään tietoja käyttäjästä. Asetukset ja
...
tarvisisi toistaa enää istuntoon liittyvää tietoa vaan ainoastaan
sivukohtaiset muutokset. Esimerkiksi HTML"-kielellä kirjoitettu
toimintolinkki \verb!<a
href="?action=x">!""\verb!linkkiteksti!""\verb!</a>! osoittaisi
automaattisesti samaan istuntoon ja samaan dokumenttiin ja selain
lähettäisi linkin mukana parametrin $action$ arvolla ``x''. Nykyisin
kaikissa linkeissä pitää aina toistaa parametrit \code{session} ja
\code{d}. Ongelmaa ei voi kiertää esimerkiksi keksien (\alt{cookies})
avulla, koska ne eivät toimi yhteistyössä selaimen
